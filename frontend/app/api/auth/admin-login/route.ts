...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Admin Login API] Request body:', { email: body.email, password: '***' });
    console.log('[Admin Login API] Backend URL:', BACKEND_URL);
    ...
    if (!response.ok) {
      ...
      console.log('[Admin Login API] Error response:', errorText);
    ...
    console.log('[Admin Login API] Success response received');
